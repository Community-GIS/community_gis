...
RUN mkdir -p /usr/src/community_gis
...
COPY . /usr/src/community_gis/
WORKDIR /usr/src/community_gis
...
RUN chmod +x /usr/src/community_gis/tasks.py \
    && chmod +x /usr/src/community_gis/entrypoint.sh
...
ENTRYPOINT service cron restart && service memcached restart && /usr/src/community_gis/entrypoint.sh
